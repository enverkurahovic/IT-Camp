...
function izdvojiPodstring(A, N, M) {
  let B = A.substring(N, N + M);

  return B;
}

let stringA = "Pera ima devojku";
let pozicijaN = 5;
let duzinaM = 7;

let podstringB = izdvojiPodstring(stringA, pozicijaN, duzinaM);
console.log(podstringB);
...
function sifrujString(originalniString) {
  let karakteri = originalniString.split("");

  for (let i = 0; i < karakteri.length - 1; i += 2) {
    [karakteri[i], karakteri[i + 1]] = [karakteri[i + 1], karakteri[i]];
  }

  return karakteri.join("");
}

let originalniString = "Pera ima devojku";
let sifrovanString = sifrujString(originalniString);
